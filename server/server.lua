...
RSGCore.Commands.Add('jail', Lang:t('lang20'), {{name = 'id', help =  Lang:t('lang21')}, {name = 'time', help = Lang:t('lang22')}}, true, function(source, args)
    ...
    local Player = RSGCore.Functions.GetPlayer(src)
        if Player.PlayerData.job.type == 'leo' then
            ...
            if time > 0 then
                TriggerClientEvent('rsg-lawman:client:jailplayer', src, playerId, time, location)
            else
...
-- jail player
--------------------------------------------------------------------------------------------------
RegisterNetEvent('rsg-lawman:server:jailplayer', function(playerId, time, location)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    ...
    if currentDate.day == 31 then
        ...
        if Player.PlayerData.job.type == 'leo' then
            if OtherPlayer then
                ...
                OtherPlayer.Functions.SetMetaData('jaillocation', location)
                OtherPlayer.Functions.SetMetaData('criminalrecord', { ['hasRecord'] = true, ['date'] = currentDate })
                TriggerClientEvent('rsg-lawman:client:sendtojail', OtherPlayer.PlayerData.source, time, location)
